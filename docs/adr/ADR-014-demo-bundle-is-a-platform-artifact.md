...
Source는 palette, typography, spacing, radius, mood와 component semantic contract를 제공하지만 Figma geometry, icon asset, CSS implementation까지 제공하지 않는다. 따라서 pixel renderer는 AAWP가 versioned console/mobile surface adapter로 소유한다. Adapter는 source token과 component definition의 props·variants·states를 validation input으로 사용하고 지원하지 않는 component type을 거부한다. Prop별 field coverage는 별도 verifier가 판정해야 한다. Adapter가 바뀌면 같은 input의 시각 결과가 달라질 수 있으므로 screen artifact가 adapter ID와 version을 고정한다.

Pixel specification이 별도로 제공되면 source spec을 수정하지 않고 immutable presentation contract로 저장한다. 우선순위는 다음과 같다.

1. 화면 의미, panel 종류, copy, state와 interaction은 pinned source spec
2. 색상, typography, shape, spacing과 responsive rule은 pinned presentation contract
3. 양쪽에 없는 pixel detail만 versioned surface adapter default
