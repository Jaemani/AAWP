...
- Input과 stdout/stderr는 단일 영속 root인 `runs/<runId>`에 보존한다.
...
현재 local executor는 단일 Studio process 안에서 실행되며 child filesystem write를 WIR capability대로 OS 수준에서 격리하지 않는다. 신뢰한 local manifest와 command 자체 sandbox만 사용한다. Server crash 뒤 자동 resume, 분산 lease, default-deny secret/environment broker, 승인·cancel과 외부 side-effect recovery는 Temporal runtime과 sandbox gateway binding이 담당해야 하며 이 ADR이 그 기능을 대체하지 않는다.
