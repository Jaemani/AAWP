...
Catalog는 executor를 새로 만들지 않는다. 현재 `spec-feedback-to-spec`도 typed launcher와 신뢰한 execution manifest를 갖춘 실행 가능 workflow다. Baseline과 feedback을 digest로 고정한 뒤 deterministic contract compile, production Codex proposal, immutable child materialization, structural·semantic verification을 순서대로 수행한다. Verifier가 거부하면 run은 `failed`로 남고 proposal과 candidate는 진단 artifact로 보존되며 성공으로 대체되지 않는다.

현재 실행 경계는 local Codex 설치·인증, 신뢰한 manifest, 단일 process와 filesystem 권한에 의존한다. 검증 finding만 대상으로 한 bounded repair finalizer는 이번 fixture의 복구·평가 도구이며 아직 catalog의 범용 자동 repair node가 아니다. Remote worker, tenant isolation, secret broker, pause/cancel, durable resume와 사람 승인·promotion UI도 아직 제공하지 않는다.
...
- Spec feedback launcher가 baseline·feedback digest, 목표 성숙도와 허가 경계를 고정하는 test
- Production proposal 실패와 verifier 거부가 run 실패 및 진단 artifact로 보존되는 local process test
