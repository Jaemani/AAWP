...
Studio 후보는 source 파일, runtime 또는 database를 직접 변경하지 않는다. browser-side 정규화는 편집 편의를 위한 것이며 publish 판단은 서버 측 compiler 결과만 사용한다.
...
- M9 focused Vitest: 7개 test file, 17개 test 통과
- 전체 Vitest: 41개 test file, 217개 test 통과
