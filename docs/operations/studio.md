...
`spec-to-demo` launcher는 `sourcePath`, `screenIds[]`, `requestText`를 받는다. Source는 서버 project root 내부의 상대경로여야 한다. 절대경로, `..` 탈출, workspace 밖을 가리키는 symlink와 source에 없는 screen ID는 process 시작 전에 거부한다. 성공하면 선택 screen의 semantic dependency closure, selection contract와 현재 `DESIGN.md` digest를 `runs/requests/<requestId>`에 기록한다. Flow·acceptance가 요구하는 screen이 빠지면 `compile-demo-scope`가 model 호출 전에 `scope-expansion-required`로 실패하고 누락 stable ID를 artifact에 남긴다.
