...
Studio 상세 화면은 workflow node 순서를 항상 표시하고, 웹 데모가 있는 run은 dashboard 안에서 미리 본다. `Open demo`는 run ID가 포함된 독립 주소를 열고 `Delete result`는 해당 snapshot을 삭제한다. 같은 workflow를 다시 실행하면 새 run ID로 결과 snapshot을 다시 만들 수 있다.
