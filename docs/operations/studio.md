...
- `npm run reverify:spec-to-demo -- run_<id>`는 immutable Demo를 현재 verifier로 다시 검사해 별도 digest-bound verdict를 만든다. 상세 API는 최신 reverify summary를 투영하되 원 run status는 바꾸지 않는다.
...
- `POST /api/runs/:runId/demo/onboard`는 completed run 또는 current verifier reverify를 통과한 저장 snapshot을 run ID URL에서 제공한다.
...
`workflows/catalog.json`은 WIR path, optional execution manifest path, 표시명·설명, input kind와 실행 bundle이 없는 경우의 이유를 등록한다. Manifest가 없는 entry는 graph inspection만 가능하며 POST Run을 거부한다. 현재 `spec-to-demo`와 `spec-feedback-to-spec`은 local process로 executable이다. `spec-feedback-to-spec` 결과는 사람이 승인하기 전 candidate이며 Studio diff/approval·promotion UI는 아직 없다. `spec-to-preview`는 Data/API contract가 S2 ready가 될 때까지 catalog executable로 등록하지 않는다.

`spec-to-demo` launcher는 `sourcePath`, optional `entryScreenId`, `screenIds[]`, `requestText`를 받는다. Entry를 생략하면 `scope.entryScreenId`를 사용하며 screen 배열 첫 항목으로 추정하지 않는다. Source는 서버 project root 내부의 상대경로여야 한다. 절대경로, `..` 탈출, workspace 밖을 가리키는 symlink와 source에 없는 screen ID는 process 시작 전에 거부한다. 성공하면 선택 screen의 semantic dependency closure, selection contract와 현재 `DESIGN.md` digest를 `runs/requests/<requestId>`에 기록한다. Flow·acceptance가 요구하는 screen이 빠지면 `scope-expansion-required`, deprecated screen·active acceptance/storyboard·journey 또는 entry가 canonical scope와 충돌하면 `selection-conflict`로 모델 호출 전에 실패하고 stable code를 artifact에 남긴다.

Demo snapshot materialization은 Demo output workflow에만 적용한다. `spec-feedback-to-spec`처럼 JSON/문서 artifact만 출력하는 workflow는 `artifacts/demo/index.html`을 요구하지 않으며, 성공 run의 `resultBuild.status`는 `not_applicable`이다.
