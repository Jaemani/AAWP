...
const source = JSON.parse(await readFile("refined-production-spec.json", "utf8")) as unknown;
const intent = parseSpecFeedbackIntent(
  JSON.parse(await readFile("examples/heavy-spec-feedback-revision/feedback-intent.json", "utf8"))
);

describe("heavy spec feedback intent", () => {
  it("pins the tested source and compiles 13 stable feedback IDs", () => {
